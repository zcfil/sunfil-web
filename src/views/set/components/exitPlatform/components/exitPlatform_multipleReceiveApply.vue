<template>
  <!-- 接收多签退出申请 -->
  <div id="exitMultipleReceiveApply" class="container">
    <!-- 退出 -->
    <div class="topProd">
      <h4>{{ $t('Exit') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            <!-- 设置操作员 -->
            <p style="margin-top: 10px">{{ $t('Exit_Request') }}</p>
            <p class="Original" style="font-weight: 700">Original Owner</p>
          ...
          <div class="addressBox">
            <p class="Original">{{ $t('Exit_Please') }}</p>
          ...
          <div v-if="itemIndex >= 0" class="lotus">
            <p class="Original">{{ $t('Exit_Message') }}</p>
          ...
          <div v-if="itemIndex >= 0" class="result">
            <p class="Original">{{ $t('Exit_Results') }}</p>
...
const { minerObj, minerEnterContractId, method, linkHref } =
...
onMounted(async () => {
  if (buffStore.departObj && accountStore.isConnected) {
    ...
    form.value.owner = buffStore.departObj.oldOwner //多签钱包地址

    node.value = buffStore.departObj.actor // 节点号
    obj.value.To = buffStore.departObj.actor //节点号

    // getGas  发送签名请求给 owner
    query2.value.params[0].To = buffStore.departObj.oldOwner

    // 多签钱包信息
    await getData(buffStore.departObj.oldOwner)
    // owner离职
    if (minerStore.minerObj.role == 1) {
      await getOwnerParams() // getGas 参数--owner
      getSerializeParams()
    } else {
      // 受益人离职
      await getAbiCborCode2()
      getSerializeParams2()
    }
  } else {
    router.push({
...
const obj = ref({
  ...
  Value: '0',
  Method: 23, //
  Params: '' // 离职方法+节点号生成的参数
})

const getOwnerParams = () => {
  const params = ownerParams(buffStore.departObj.oldOwnerId)
  obj.value.Params = params
  // console.log('getOwnerParams:', params)
}

const getSerializeParams = () => {
  ...
  // console.log('getSerializeParams:', params)
}

// getGas  params 参数--受益人
const node = ref()
const signQuery2 = ref('minerExitingBeneficiary(uint64)') //智能合约abi接口  方法+节点号
const getAbiCborCode2 = async () => {
  const params = departAbi(signQuery2.value, node.value)
  obj2.value.Params = params
}

const obj2 = ref({
  To: minerStore.minerEnterContractId, //合约id
  Value: '0',
  Method: minerStore.method, // 3844450837,
  Params: '' // 离职方法+节点号生成的参数
})

const getSerializeParams2 = () => {
...
const getTxIdParams = id => {
  const params = txIdParams(id) // 生成编码...
  ...
  if (!res.error) {
    ...
    query3.value = await {
      ...
      params: [
        {
          Message: {
            From: res.result.From, //成员地址
            Method: res.result.Method, // 2提案 3同意
            Nonce: res.result.Nonce, //nonce值
            Params: res.result.Params,
  ...
  if (!res.error) {
    if (res.result.Receipt.ExitCode == 0) {
      ...
      if (+sendNum.value < +form.value.threshold) {
        ...
        // 去修改 owner
        await getMinerInfo(buffStore.departObj.actor)
