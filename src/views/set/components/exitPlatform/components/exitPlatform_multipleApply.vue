<template>
  <!-- 多签 -->
  <div id="exitMultipleApply" class="container">
    <!-- 退出 -->
    <div class="topProd">
      <h4>{{ $t('Exit') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            <!-- 退出申请 -->
            <p style="margin-top: 10px">{{ $t('Exit_Submit') }}</p>
...
const { minerEnterContractId, method, minerObj, linkHref } =
...
onMounted(async () => {
  if (buffStore.actorInfo && accountStore.isConnected && minerStore.minerObj) {
    form.value.node = buffStore.departObj.actor // 节点号
    form.value.owner = buffStore.departObj.owner //多签钱包地址
...
//获取 owner  params参数
const signQuery = ref('minerExiting(uint64)') //智能合约abi接口  方法+节点号
const node = ref()
...
const getBeneficiaryParams = () => {
  const params = beneficiaryParams2(minerStore.minerObj.owner) //用owner地址
...
const getSerializeParams2 = async () => {
  ...
  // console.log('getSerializeParams2:', params)
}

// 获取二次及以上签名 getGas 参数
const getTxIdParams = id => {
  // 生成编码...
  const params = txIdParams(id)
...
const getMsg = async () => {
  ...
  if (!res.error) {
    if (res.result.Receipt.ExitCode == 0) {
      ...
      router.push(`/buff/minerInfo/${form.value.node}`)
    }
  } else {
    // 上链失败
    loading.value = false //停止加载
