<template>
  <!-- 多签 -->
  <div id="singleReceiveApply" class="container">
    <div class="topProd">
      <h4>{{ $t('Exit') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <!-- 申请 -->
          <div class="apply">
            <p style="margin-top: 10px">{{ $t('Check_Request1') }}</p>
          </div>
...
const { departObj } = storeToRefs(buffStore) //解构
...
onMounted(async () => {
  if (accountStore.isConnected && minerStore.minerObj) {
    ...
    //   已离职但owner或受益人未改--- getNonce
    if (buffStore.departObj.status == 2 || buffStore.departObj.status == 3) {
      query.value.params = [buffStore.departObj.oldOwner] //老owner地址
    } else {
      // owner地址 拿到nonce值
      query.value.params = await [minerStore.minerObj.owner]
    }
    ...
    if (minerStore.minerObj.role == 1) {
      query2.value.params[0].From = buffStore.departObj.oldOwner //老owner地址
      query2.value.params[0].Method = 23
      query2.value.params[0].To = minerStore.minerObj.node //节点号
      await getOwnerParams() // getGas 参数--owner
    } else {
      query2.value.params[0].From = minerStore.minerObj.oldOwner
      query2.value.params[0].Method = minerStore.method
      query2.value.params[0].To = minerStore.minerEnterContractId //合约id
      await getBeneficiaryAbi() // getGas 参数--受益人
    }
...
const getBeneficiaryAbi = () => {
  const params = departAbi(signQuery2.value, node.value)
  query2.value.params[0].Params = params

  // console.log('getBeneficiaryAbi:', params)
}
      ...
      params: [
        {
          Message: {
            ...
            Method: res.result.Method, //方法类型，3代表同意签名
