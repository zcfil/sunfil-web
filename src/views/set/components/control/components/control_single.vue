<template>
  <div id="ControlSingle" class="container">
    <!-- 修改Control -->
    <div class="topProd">
      <h4>{{ $t('Control') }}</h4>
...
onMounted(async () => {
  if (minerStore.minerObj) {
    ...
    controlAddArr.value = await setStore.controlInfo.controlArr // 控制地址数组
...
const getNonce = async () => {
  query.value.params = await [minerStore.minerObj.oldOwner] //用oldOwner钱包地址
...
const getGas = async () => {
  query2.value.params[0].From = await minerStore.minerObj.oldOwner //用oldOwner钱包地址
