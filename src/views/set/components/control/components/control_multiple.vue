<template>
  <!-- 多签 -->
  <div id="ControlMultiple" class="container">
    <!-- 修改Control -->
    <div class="topProd">
      <h4>{{ $t('Control') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            <!-- 设置操作员 -->
            <p style="margin-top: 10px">{{ $t('Control_change') }}</p>
...
onMounted(async () => {
  if (
    minerStore.minerObj == 'undefined' ||
    minerStore.minerObj == '' ||
    minerStore.minerObj == null
  ) {
    router.push({
      path: '/buff/buff_home'
    })
    ...
    form.value.owner = minerStore.minerObj.oldOwner

    node.value = minerStore.minerObj.node //当前节点号

    // 有新的就用新的  没有就用旧的
    if (
      setStore.controlInfo.NewWorker !== '<empty>' &&
      setStore.controlInfo.NewWorker
    ) {
      workerId.value = setStore.controlInfo.NewWorker
    } else {
      workerId.value = setStore.controlInfo.Worker
    }

    // 多签钱包信息
    getData(minerStore.minerObj.oldOwner)

    const controlArr = setStore.controlInfo.controlArr // 新增控制地址数组
    const oldControlArr = ref()
    // control地址有就传，没有就传空
    if (setStore.controlInfo.ControlAddresses) {
      oldControlArr.value = setStore.controlInfo.ControlAddresses.map(v =>
        v.substr(2)
      ) // 控制地址数组

      const concatArr = oldControlArr.value.concat(controlArr) //合并数组
      controlAddArr.value = arrFn(concatArr) // 数组去重
    } else {
      controlAddArr.value = controlArr
    }

    // 获取 gas 参数  发送签名请求给 owner
    query2.value.params[0].To = minerStore.minerObj.oldOwner

    await getAbiCborCode()
    getSerializeParams()
  }
})
...
// 获取合约 abi编码params
const signQuery = ref('changeWorkerAddress(uint256,uint256,uint256[])') //智能合约abi接口  节点号 + worker钱包ID + control地址数组
const node = ref()
const workerId = ref()
const controlAddArr = ref()
const getAbiCborCode = async () => {
  const arr = [signQuery.value, node.value, workerId.value, controlAddArr.value]
  console.log('arr:', arr)
  const params = contractAbiCborCode3(arr)
...
// getGas  params 参数 第一次
const obj = ref({
  To: minerStore.minerEnterContractId, //合约 id
  Value: '0',
  Method: 3844450837,
  Params: '' //合约地址编码
})
      ...
      params: [
        {
          Message: {
            ...
            Params: res.result.Params,
            To: minerStore.minerObj ? minerStore.minerObj.oldOwner : '', //多签钱包地址
            Value: '0', //金额0
            GasFeeCap: res.result.GasFeeCap,
            GasLimit: res.result.GasLimit,
            GasPremium: res.result.GasPremium,
