<template>
  <div id="SetOwnerSingleInfo" class="container">
    <!-- 设置操作员 -->
    <div class="topProd">
      <h4>{{ $t('operator') }}</h4>
...
onMounted(async () => {
  if (setStore.operatorInfo && accountStore.isConnected) {
    node.value = setStore.operatorInfo.actor
    newOperatorAddress.value = setStore.operatorInfo.newOperator
    // getGas参数
    query2.value.params[0].From = setStore.operatorInfo.owner
    query2.value.params[0].Nonce = setStore.operatorInfo.nonce
