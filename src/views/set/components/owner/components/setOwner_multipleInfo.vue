<template>
  <!-- 多签 -->
  <div id="SetOwnerMultipleInfo" class="container">
    <!-- 设置操作员 -->
    <div class="topProd">
      <h4>{{ $t('operator') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            <!-- 设置操作员 -->
            <p style="margin-top: 10px">{{ $t('operator_operator') }}</p>
...
const { minerEnterContractId, minerObj, linkHref } = storeToRefs(minerStore)
...
onMounted(async () => {
  if (setStore.operatorInfo) {
    form.value.owner = setStore.operatorInfo.owner
    form.value.node = setStore.operatorInfo.actor

    node.value = setStore.operatorInfo.actor
    accountAddress.value = setStore.operatorInfo.newOperator

    // 获取 gas 参数  发送签名请求给 owner
    query2.value.params[0].To = setStore.operatorInfo
      ? setStore.operatorInfo.owner
      : ''
...
const minerData = ref()
const getData = async (
  owner = setStore.operatorInfo ? setStore.operatorInfo.owner : ''
) => {
...
// 获取合约 abi编码params
const signQuery = ref('setOperator(uint64,address)') //智能合约abi接口  节点号 + 新操作人地址
const node = ref()
      ...
      params: [
        {
          Message: {
            ...
            Params: res.result.Params,
            To: setStore.operatorInfo ? setStore.operatorInfo.owner : '', //多签钱包地址
            Value: '0',
            GasFeeCap: res.result.GasFeeCap,
            GasLimit: res.result.GasLimit,
            GasPremium: res.result.GasPremium,
...
const addressClick = async (item, index) => {
  ...
  if (!loading.value) {
    ...
    query.value.params = [item.Address] //钱包地址
