<template>
  <!-- 多签 -->
  <div id="WorkerMultiple" class="container">
    <!-- 修改worker -->
    <div class="topProd">
      <h4>{{ $t('worker') }}</h4>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            <!-- 退出申请 -->
            <p style="margin-top: 10px">{{ $t('worker') }}</p>
...
const { operatorInfo, clearSet, controlInfo } = storeToRefs(setStore)

const minerStore = useMinerStore()
const { minerEnterContractId, minerObj, method, linkHref } =
...
onMounted(async () => {
  if (minerStore.minerObj) {
    form.value.node = setStore.controlInfo.node // 节点号
    form.value.owner = minerStore.minerObj.owner //多签钱包地址

    node.value = minerStore.minerObj.node
    workerId.value = minerStore.minerObj.newWorkerId
...
// 如果 newWorker 为空获取合约 abi编码params
const node = ref()
const workerId = ref() //新worker
