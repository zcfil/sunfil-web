<template>
  <!-- 接收多签退出申请 -->
  <div id="WorkerMultipleConfirm" class="container">
    <!-- 退出 -->
    <div class="topProd">
      <h4>{{ $t('Exit') }}</h4>
...
onMounted(async () => {
  if (buffStore.actorInfo && accountStore.isConnected && minerStore.minerObj) {
    form.value.node = minerStore.minerObj.node
    form.value.owner = minerStore.minerObj.oldOwner //多签钱包地址
    form.value.oldAddress = setStore.controlInfo.NewWorker //待确认新worker

    // getGas  发送签名请求给 老owner
    query2.value.params[0].To = minerStore.minerObj.oldOwner
    node.value = minerStore.minerObj.node // 节点号

    // 多签钱包信息
    await getData(minerStore.minerObj.oldOwner)
    await getAbiCborCode() //新worker为空的时候
    getSerializeParams()
  } else {
    router.push({
...
// 如果 newWorker 有值
const node = ref()
const signQuery = ref('confirmChangeWorkerAddress(uint64)') //智能合约abi接口  节点号ID
const getAbiCborCode = () => {
  const arr = [signQuery.value, node.value]

  const params = contractAbiCborCode4(arr)
  obj.value.Params = params //  getGas参数
  // console.log('getAbiCborCode:', params)
}
...
const handleCopy = async val => {
  if (val == 'old') {
    try {
      await toClipboard(form.value.oldAddress)
