<template>
  <!-- 单签 -->
  <div id="SingleApply" class="container">
    <!-- 节点入驻介绍 -->
    <div class="topProd">
      <h4>{{ $t('Check') }}</h4>
      <p>
    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <!-- 申请 -->
          <div class="apply">
            <p v-if="minerStore.minerObj.role == 1" style="margin-top: 10px">
              {{ $t('Check_Submit') }}
            </p>
            <p v-if="minerStore.minerObj.role == 2" style="margin-top: 10px">
              {{ $t('Check_Submit2') }}
            </p>
...
onMounted(async () => {
  if (minerStore.minerObj && minerStore.checkInBool && !minerStore.nodeDebt) {
    ...
    if (minerStore.minerObj.role == 1) {
      ...
      query2.value.params[0].Params = minerStore.contractObj.Params //合约参数
    } else {
      // 受益人
  ...
  params: [
    {
      From: '', //owner钱包地址
      To: '', //节点号
      Method: 0, //方法类型，23代表修改owner(入职)  30代表受益人
      Params: '', //3.生成的param参数
      Value: '0', //金额0
      Nonce: '', // 获取的nonce值
      Version: 0, //0
...
const cidParams = ref()
const getGas = async () => {
  query2.value.params[0].From = await minerStore.minerObj.owner //钱包地址
  query2.value.params[0].To = await minerStore.minerObj.node //节点号
