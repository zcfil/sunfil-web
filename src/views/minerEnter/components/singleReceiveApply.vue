    ...
    <div class="bottomCard">
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <!-- 申请 -->
          <div class="apply">
            <p style="margin-top: 10px">{{ $t('Check_Request') }}</p>
          </div>
...
onMounted(async () => {
  if (minerStore.minerObj && minerStore.checkInBool && !minerStore.nodeDebt) {
    ...
    //操作人地址
    accountAddress.value = minerStore.minerObj.operatorAddress

    //用查询的owner钱包地址 拿到nonce值
    query.value.params = await [minerStore.minerObj.owner]
    query2.value.params[0].From = minerStore.minerObj.owner
    query2.value.params[0].To = minerStore.minerEnterContractId
    query2.value.params[0].Method = minerStore.method //3844450837

    await getNonce()
    if (minerStore.minerObj.role == 1) {
      await getAbiCborCode() // getGas 参数--owner
    } else {
      await getAbiCborCode2() // getGas 参数--受益人
    }
    getGas()
  } else {
    router.push({
...
// 获取合约 abi编码params---owner入职
const signQuery = ref('minerJoining(uint64,address)') //智能合约abi接口
const node = ref() //节点号
