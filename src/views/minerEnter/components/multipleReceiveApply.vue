<template>
  <!-- 接受多签申请 -->
  <div id="MultipleReceiveApply" class="container">
    <!-- 节点入驻介绍 -->
    <div class="topProd">
      <h4>{{ $t('Check') }}</h4>
      <p>
      ...
      <div class="cardInfo col-8 col-sm-8 col-md-8 col-lg-8">
        <div class="contentBox">
          ...
          <div class="apply">
            ...
            <p
              v-if="minerStore.minerObj.role == 1"
              class="Original"
            ...
            <p
              v-if="minerStore.minerObj.role == 2"
              class="Original"
              style="font-weight: 700"
            >
          ...
          <div v-if="itemIndex >= 0" class="infoContent">
            <p class="Original">{{ $t('Check_Requestcontent') }}</p>
          ...
          <div v-if="itemIndex >= 0" class="lotus">
            <p class="Original">{{ $t('Check_RequestMessage') }}</p>
...
onMounted(async () => {
  ...
  if (minerStore.minerObj && minerStore.checkInBool && !minerStore.nodeDebt) {
    ...
    accountAddress.value = minerStore.minerObj.operatorAddress //操作人地址

    // 获取 gas 参数  发送签名请求给 owner
    query2.value.params[0].To = minerStore.minerObj.owner

    // 多签钱包信息
    getData(minerStore.minerObj.owner)
...
// 获取合约 abi编码params
const signQuery = ref('minerJoining(uint64,address)') //智能合约abi接口  节点号 + 操作人地址
const node = ref() //节点号
const accountAddress = ref() //操作人地址
const getAbiCborCode = async () => {
  const params = contractAbiCborCode(
    signQuery.value,
    node.value,
...
// 获取合约 abi编码params---受益人入职
const signQuery2 = ref('minerJoiningBeneficiary(uint64,address)') //智能合约abi接口 节点号+操作人地址
const getAbiCborCode2 = async () => {
  ...
  // console.log('getAbiCborCode2:', params)
}
...
const getMsg = async () => {
  ...
  if (!res.error) {
    if (res.result.Receipt.ExitCode == 0) {
      ...
      router.push('/buff/minerRoleInfo')
    }
  } else {
    // 上链失败
    loading.value = false //停止加载
